...
**課題**:
Service層の単体テスト（Mock使用）は全てパスしているにも関わらず、結合動作時にリクエストボディのバインディング（Ginの `ShouldBindJSON`）が意図せず失敗するバグがありました。

**対応**:
単体テストではHTTPリクエストのコンテキストまでカバーしきれないことを痛感しました。
- **修正**: ハンドラー層でのバリデーションロジックを見直し、正常に動作するように修正。
- **学び**: E2Eテストや統合テストの重要性を認識しました。現在は優先度ごとの実装計画に見直し、まずは主要機能の統合テスト導入を検討しています。
