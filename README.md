...
## 6. 技術選定と設計思想
#### 🧭 ユーザー心理に基づいたMVP策定（羅針盤としての要件定義）
開発初期にペルソナ（お金の現状把握が苦手なユーザー）を深く分析しました。「高機能な家計簿」ではなく「開いた瞬間に現状を把握できるツール」が必要だと定義し、**あえて「細かい分析機能」をMVPから削り、「残額表示」に特化**させました。製品のコンセプトとして
> “今の自分のお金の状況が一目でわかるアプリ” 
> 
> シンプルな入力とリアルタイムの残額表示で、貯まる行動を自然に習慣化する。

を最初に指針として定めていたため、ブレずに開発を完遂できました。
#### 🛡️ クリーンアーキテクチャ × sqlc による堅牢性
Goの強みである型安全性を最大限活かすため、ORM（GORMなど）ではなく **sqlc** を採用しました。
- **意図**: 複雑な集計クエリ（残額計算など）をSQLで直接書きつつ、Goのコード側では型安全を保証するため。
- **構成**: レイヤードアーキテクチャを採用し、ビジネスロジックとDBアクセスを疎結合に保つことで、テスト容易性と保守性を高めています。

## 開発で直面した課題と解決策 (Technical Challenges)
...
- **結果**: 接続プールの最適化により、連続して読み込みをしても接続エラーがゼロになり、レスポンス速度も安定しました。これにより「ライブラリ選定がインフラとの相性にどう影響するか」を深く学ぶことができました。

### 🐛 2. 単体テストの死角とバインディング挙動の修正
**課題**:
Service層の単体テスト（Mock使用）は全てパスしているにも関わらず、結合動作時にService層のバリデーションが意図せず失敗するバグがありました。

**原因**
Service層のテストでは、Handlerを通さず、構造体を直接生成してServiceを呼んでいたため、Goのbindingタグによる挙動を一切通っていませんでした。
`binding:"required"` は 0 を未入力とみなすため、意図せずバリデーションエラーが起きていました。

**解決策**:
`binding:"required"`を使用しないように変更しました。
- **結果**: 意図しないバリデーションエラーが出ないようになりました。
- **学び**: E2Eテストや統合テストの重要性を認識しました。現在は優先度ごとの実装計画に見直し、主要機能の統合テスト導入を検討しています。
