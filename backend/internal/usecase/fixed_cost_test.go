...
func (m *mockFixedCostRepo) CreateFixedCost(ctx context.Context, userID string, name string, amount int) (domain.FixedCost, error) {
	args := m.Called(ctx, userID, name, amount)
	return args.Get(0).(domain.FixedCost), args.Error(1)
}

func (m *mockFixedCostRepo) ListFixedCostsByUser(ctx context.Context, userID string) ([]domain.FixedCost, error) {
	args := m.Called(ctx, userID)
	if result := args.Get(0); result != nil {
		return result.([]domain.FixedCost), args.Error(1)
	}
...
func (m *mockFixedCostRepo) BulkCreateFixedCosts(ctx context.Context, userID string, fixedCosts []FixedCostItem) error {
...
func TestListFixedCosts(t *testing.T) {
	...
	t.Run("正常に固定費一覧を取得できる", func(t *testing.T) {
		...
		uc := NewListFixedCostsUseCase(repo)
		result, err := uc.Execute(ctx, "user1")
	...
	t.Run("空の一覧を取得できる", func(t *testing.T) {
		repo := new(mockFixedCostRepo)
		repo.On("ListFixedCostsByUser", ctx, "user1").Return([]domain.FixedCost{}, nil)
...
func TestCreateFixedCost(t *testing.T) {
	...
	t.Run("正常に固定費を作成できる", func(t *testing.T) {
		...
		uc := NewCreateFixedCostUseCase(repo)
		result, err := uc.Execute(ctx, "user1", "家賃", 80000)
	...
	t.Run("名前が空の場合はエラー", func(t *testing.T) {
		...
		uc := NewCreateFixedCostUseCase(repo)
		_, err := uc.Execute(ctx, "user1", "", 80000)
	...
	t.Run("名前が最大長を超える場合はエラー", func(t *testing.T) {
		...
		uc := NewCreateFixedCostUseCase(repo)
		_, err := uc.Execute(ctx, "user1", longName, 80000)
	...
	t.Run("金額が0以下の場合はエラー", func(t *testing.T) {
		...
		uc := NewCreateFixedCostUseCase(repo)
		_, err := uc.Execute(ctx, "user1", "家賃", 0)
	...
	t.Run("金額が上限を超える場合はエラー", func(t *testing.T) {
		...
		uc := NewCreateFixedCostUseCase(repo)
		_, err := uc.Execute(ctx, "user1", "家賃", BusinessMaxAmount+1)
...
func TestUpdateFixedCost(t *testing.T) {
	...
	t.Run("正常に固定費を更新できる", func(t *testing.T) {
		...
		repo.On("ListFixedCostsByUser", ctx, "user1").Return([]domain.FixedCost{
			{ID: 1, UserID: "user1", Name: "家賃（更新）", Amount: 85000},
		}, nil)

		uc := NewUpdateFixedCostUseCase(repo)
		result, err := uc.Execute(ctx, "user1", 1, "家賃（更新）", 85000)
	...
	t.Run("名前が空の場合はエラー", func(t *testing.T) {
		...
		uc := NewUpdateFixedCostUseCase(repo)
		_, err := uc.Execute(ctx, "user1", 1, "", 80000)
	...
	t.Run("名前が最大長を超える場合はエラー", func(t *testing.T) {
		...
		uc := NewUpdateFixedCostUseCase(repo)
		_, err := uc.Execute(ctx, "user1", 1, longName, 80000)
	...
	t.Run("金額が0以下の場合はエラー", func(t *testing.T) {
		...
		uc := NewUpdateFixedCostUseCase(repo)
		_, err := uc.Execute(ctx, "user1", 1, "家賃", 0)
	...
	t.Run("金額が負の値の場合はエラー", func(t *testing.T) {
		...
		uc := NewUpdateFixedCostUseCase(repo)
		_, err := uc.Execute(ctx, "user1", 1, "家賃", -1000)
	...
	t.Run("金額が上限を超える場合はエラー", func(t *testing.T) {
		...
		uc := NewUpdateFixedCostUseCase(repo)
		_, err := uc.Execute(ctx, "user1", 1, "家賃", 1000000001)
	...
	t.Run("更新後に固定費が見つからない場合はエラー", func(t *testing.T) {
		...
		uc := NewUpdateFixedCostUseCase(repo)
		_, err := uc.Execute(ctx, "user1", 1, "家賃", 80000)
...
func TestDeleteFixedCost(t *testing.T) {
	...
	t.Run("正常に固定費を削除できる", func(t *testing.T) {
		...
		uc := NewDeleteFixedCostUseCase(repo)
		err := uc.Execute(ctx, "user1", 1)
