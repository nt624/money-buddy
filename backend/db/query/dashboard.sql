...
LEFT JOIN monthly_settings ms
  ON ms.user_id = u.id AND ms.year = sqlc.arg(year)::int AND ms.month = sqlc.arg(month)::int
...
WHERE e.user_id = sqlc.arg(user_id)
  AND DATE_TRUNC('month', e.spent_at) = DATE_TRUNC('month', MAKE_DATE(sqlc.arg(year)::int, sqlc.arg(month)::int, 1));